...
/**
 * Hero de saldo del cliente. Cumple el requisito:
 *  "Tenés 100 Huellitas (Equivalen a $1.000 de descuento)"
 */
export function HuellitasBalance({
  saldo,
  ...
  return (
    <div className="surface-card relative overflow-hidden p-8">
      ...
      <div className="absolute right-6 top-6 flex gap-1.5 opacity-25">
        <HuellitaIcon size={32} className="text-bark-400 rotate-12" />
        <HuellitaIcon size={20} className="text-bark-400 -rotate-6" />
      </div>
      ...
      <div className="relative">
        ...
        <HuellitasStack count={Math.min(saldo, 6)} className="mt-7" size={24} />
      </div>
    </div>
  );
}
