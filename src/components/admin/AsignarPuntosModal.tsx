...
export function AsignarPuntosModal({
  cliente,
          ...
          <div>
            ...
            <p className="text-xs text-bark-400">
              ...
              <span className="font-semibold text-bark-700">
                {formatNumber(cliente.saldoHuellitas)} puntos
              </span>
            </p>
          </div>
          <button
          ...
          <form onSubmit={onSubmit} className="mt-6 space-y-4">
            <label className="block">
              ...
              <p className="mt-2 text-xs text-bark-400">
                1 punto por cada {formatARS(montoParaUnaHuellita)} · Canje{" "}
                {formatARS(valorMonetarioHuellita)}/punto
              </p>
            </label>
                ...
                <div>
                  ...
                  <div className="mt-1 flex items-baseline gap-2">
                    <span className="font-display text-4xl font-bold tabular-nums text-bark-700">
                      {formatNumber(puntosPrevistos)}
                    </span>
                    <span className="text-sm text-bark-500">puntos</span>
                  </div>
                </div>
                <PuntoIcon
                  size={36}
                  className={
                    puntosPrevistos > 0 ? "text-bark-500" : "text-bark-200"
                  }
                />
              </div>
            </div>
          ...
          <div className="mt-6 text-center">
            ...
            <div className="mt-4 rounded-2xl bg-cream-50 px-5 py-4">
              ...
              <div className="mt-1 flex items-center justify-center gap-2">
                <PuntoIcon size={26} className="text-bark-500" />
                <span className="font-display text-4xl font-bold tabular-nums text-bark-700">
                  {formatNumber(resultado.saldoFinal ?? 0)}
                </span>
              </div>
              <p className="mt-1 text-xs text-bark-500">
                +{formatNumber(resultado.huellitasGeneradas ?? 0)} puntos en esta venta
              </p>
            </div>
            <button
              onClick={onClose}
...
/** @deprecated Usar `AsignarPuntosModal`. */
export const AsignarHuellitasModal = AsignarPuntosModal;
