    ...
    <section className="space-y-6">
      <header className="flex flex-wrap items-end justify-between gap-4">
        <div>
          ...
          <p className="mt-1 max-w-xl text-sm text-bark-600">
            Lectura de ventas, canjes y comunidad. Solo consulta — no modifica caja ni
            canjes.
          </p>
        </div>

        <div
          className="inline-flex rounded-2xl bg-gradient-to-br from-bark-800 to-bark-900 p-1 ring-1 ring-white/10"
          role="group"
          ...
          {RANGOS_REPORTE.map((opt) => (
            <button
              ...
              className={cn(
                ...
                rango === opt.id
                  ? "bg-gradient-to-r from-terracotta-500 to-amber-500 text-white shadow-sm"
      ...
      {data?.ventana ? (
        <p className="text-xs text-bark-500">
          Período: <strong className="text-bark-700">{data.ventana.etiqueta}</strong>
          {" · "}solo lectura
        </p>
      ) : null}
